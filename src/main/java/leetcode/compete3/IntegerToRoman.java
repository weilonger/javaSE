package main.java.leetcode.compete3;

/*题目描述
    12. 整数转罗马数字
        字符          数值
        I             1
        V             5
        X             10
        L             50
        C             100
        D             500
        M             1000
    通常情况下，罗马数字中小的数字在大的数字的右边。但也存在特例，例如 4 不写做 IIII，而是 IV。
    数字 1 在数字 5 的左边，所表示的数等于大数 5 减小数 1 得到的数值 4 。同样地，数字 9 表示为 IX。这个特殊的规则只适用于以下六种情况：
    I 可以放在 V (5) 和 X (10) 的左边，来表示 4 和 9。
    X 可以放在 L (50) 和 C (100) 的左边，来表示 40 和 90。
    C 可以放在 D (500) 和 M (1000) 的左边，来表示 400 和 900。
    给定一个整数，将其转为罗马数字。输入确保在 1 到 3999 的范围内。
 */
/*示例1：
    输入: 4
    输出: "IV"
 */
/*示例2：
    输入: 58
    输出: "LVIII"
    解释: L = 50, V = 5, III = 3.
 */
/*示例3：
    输入: 1994
    输出: "MCMXCIV"
    解释: M = 1000, CM = 900, XC = 90, IV = 4.
 */
public class IntegerToRoman {
    //105ms
    public String intToRoman(int num) {
        String[] signal = new String[]{"I","V","X","L","C","D","M"};
        if (num > 3999 || num < 1) {
            return "";
        }
        StringBuffer roman = new StringBuffer();
        int thousand = num / 1000;
        num %= 1000;
        for (int i = 0; i < thousand; i++){
            roman.append("M");
        }
        for (int i = 3; i > 0; i--) {
            int temp =  num / (int)Math.pow(10, i - 1);
            if (temp == 9) {
                roman.append(signal[i * 2 - 2]).append(signal[i * 2]);
            } else if (temp == 4) {
                roman.append(signal[i * 2 - 2]).append(signal[i * 2 - 1]);
            } else {
                if (temp >= 5) {
                    roman.append(signal[i * 2 - 1]);
                }
                for (int j = 0; j < temp % 5; j++){
                    roman.append(signal[i * 2 - 2]);
                }
            }
            num = num % (int)Math.pow(10, i - 1);
        }
        return roman.toString();
    }

    public static void main(String[] args) {
        IntegerToRoman i = new IntegerToRoman();
        int num = 1982;
        System.out.println(i.intToRoman1(num));
    }

    //101ms
    public String intToRoman1(int num) {
        int values[]={1000,900,500,400,100,90,50,40,10,9,5,4,1};
        String reps[]={"M","CM","D","CD","C","XC","L","XL","X","IX","V","IV","I"};

        StringBuffer res = new StringBuffer();
        for(int i=0; i<13; i++){
            while(num>=values[i]){
                num -= values[i];
                res.append(reps[i]);
            }
        }
        return res.toString();
    }
}
